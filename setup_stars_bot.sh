...
# Использование:
#   sudo bash setup_stars_bot.sh <VPS_USER> <APP_DIR>
# Пример:
...
# 4. Автозапуск службы (без немедленного старта)
systemctl daemon-reload
systemctl enable stars_bot.service

echo "
Сервис настроен на автозапуск после перезагрузки, но не запущен автоматически."
echo "Для запуска вручную выполните: sudo systemctl start stars_bot.service"
echo "Для проверки статуса: sudo systemctl status stars_bot.service"
